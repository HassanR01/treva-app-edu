            ...
            <ScrollView style={ConstantStyles.page}>
                {
                    searchLessons?.map((lesson, index) => {
                        return (
                            <TouchableOpacity key={index} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', margin: 10, padding: 10, borderWidth: 1, borderColor: Colors.mainColor, borderRadius: 5, direction: 'rtl' }}
                                onPress={() => router.push(
                                    {
                                        pathname: '/(course)/[id]',
                                        params: {
